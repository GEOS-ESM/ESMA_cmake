# Intel_Fortran.cmake -- Optimized for Intel compiler on AMD Milan (EPYC, AVX2/FMA3)
...
if (QUIET_DEBUG)
  set (WARN_UNUSED "")
  set (SUPPRESS_COMMON_WARNINGS "${DISABLE_FIELD_WIDTH_WARNING} ${DISABLE_GLOBAL_NAME_WARNING} ${DISABLE_10337} ${DISABLE_10121} ${DISABLE_10448}")
else ()
  set (WARN_UNUSED "-warn unused")
  set (SUPPRESS_COMMON_WARNINGS "${DISABLE_GLOBAL_NAME_WARNING} ${DISABLE_10337} ${DISABLE_10121} ${DISABLE_10448}")
endif ()
...
# For Milan: AVX2/FMA3 baseline
set (COREAVX2_FLAG "-march=core-avx2")

# Use 32-byte alignment for arrays (AVX2)
...
set (common_Fortran_fpe_flags "${NOOLD_MAXMINLOC} ${SUPPRESS_COMMON_WARNINGS}")

# ----------------------------------------------------------------------
# Build type specific bundles
# ----------------------------------------------------------------------
# Debug
set (GEOS_Fortran_Debug_Flags
     "${DEBINFO} ${FOPT0} -debug -nolib-inline -fno-inline-functions "
     "-assume protect_parens,minus0 -prec-div -prec-sqrt "
     "-check all,noarg_temp_created -fp-stack-check ${WARN_UNUSED} "
     "-init=snan,arrays -save-temps")

set (GEOS_Fortran_Debug_FPE_Flags
     "${FPE0} ${FP_STRICT} ${FP_SPECULATION_STRICT} ${common_Fortran_fpe_flags}")
...
set (GEOS_Fortran_VectTrap_FPE_Flags
     "${FP_PRECISE} ${ARCH_CONSISTENCY} ${NO_FMA} ${FPE0} -check uninit ${common_Fortran_fpe_flags}")
